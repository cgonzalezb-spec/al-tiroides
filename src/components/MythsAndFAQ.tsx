...
const MythsAndFAQ = () => {
  ...
  const clinicalRealities = ["El hipotiroidismo y el hipertiroidismo son enfermedades comunes, con causas bien definidas (autoinmunidad, deficiencia de yodo, fármacos, cirugía, etc.) y diagnóstico basado en pruebas de laboratorio (TSH, T4 libre).", "El tratamiento estándar del hipotiroidismo es la levotiroxina, que es segura, eficaz y económica. El uso de T3 o extractos desecados de tiroides no está recomendado de rutina y carece de suficiente respaldo científico.", "El hipotiroidismo subclínico solo requiere tratamiento en situaciones específicas (TSH persistentemente >10 mU/L, síntomas claros, embarazo, infertilidad, o presencia de anticuerpos antitiroideos).", "La mayoría de los nódulos tiroideos son benignos; la indicación de biopsia o derivación depende de características clínicas y ecográficas.", "El exceso de yodo puede inducir disfunción tiroidea, por lo que la suplementación debe ser individualizada."];
  const faqs = [{
  ...
  return <section id="mitos-faq" className="py-20 bg-white">
      <div className="container mx-auto px-4">
        ...
        <div className="mb-16">
          ...
          <div className="space-y-4">
            {myths.map((item, index) => <Card key={index} className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => setExpandedMyth(expandedMyth === index ? null : index)}>
                <CardHeader>
                  <div className="flex items-start justify-between">
                    ...
                    {expandedMyth === index ? <ChevronUp className="h-5 w-5 text-gray-500 mt-1" /> : <ChevronDown className="h-5 w-5 text-gray-500 mt-1" />}
                  </div>
                </CardHeader>
                {expandedMyth === index && <CardContent>
        ...
        {/* Realidades clínicas */}
        <div className="mb-16">
          <h3 className="text-3xl font-bold text-gray-900 mb-8 text-center">Realidades clínicas</h3>
          <Card className="bg-green-50 border-green-200">
            <CardContent className="pt-6">
              <ul className="space-y-4">
                {clinicalRealities.map((reality, index) => <li key={index} className="flex items-start space-x-3">
                    <CheckCircle className="h-5 w-5 text-green-600 mt-0.5 flex-shrink-0" />
        ...
        <div>
          ...
          <div className="space-y-4">
            {faqs.map((faq, index) => <Card key={index} className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => setExpandedFaq(expandedFaq === index ? null : index)}>
                <CardHeader>
